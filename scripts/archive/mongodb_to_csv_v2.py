...
def csvize_deleted():
	...
	with open("deleted_weibo.csv", "w") as wf:
		wf.write("post_id, created_at, poster_id \n")
		print "post_id, created_at, poster_id"
...
def search_date(postid):
	...
	logging.debug("=========")
	logging.debug("POSTID = " + str(postid))
...
#			logging.debug("   searching for " + str(searchid) + ": EXISTS")
#			logging.debug(rdata["created_at"])
			thiscreated = parse(rdata['created_at']).strftime('%s')
			return thiscreated

#		logging.debug("   searching for " + str(searchid )+ ": NO")
